...
class BuildChecker():
    ...
    def checkCommitHistory(self):
        ...
        for c_hash, commit in self.project.report.getRows():
            ...
            already_checked = True if commit['build'] != "NO" else False

            # CHECK BUILD

            build_config = self.checkBuild(c_hash, commit)

            # CHECK TEST COMPILE

            if commit['build'] == "SUCCESS":

                if (commit['test_build'] == "NO"):
    ...
    def checkBuild(self, c_hash, commit):
        if (commit['build'] == "NO"): # NO BUILD CHECKED
