...
class ProjectTestAnalysis():
    ...
    def generateAndSavePlot(self):
        ...
        results_df['non_buildable'] = results_df.apply(self.map_commit, axis=1)
        ...
        if results_df[results_df.n_test > 0].empty:
            pass
        else:
            inf_limit = results_df[results_df.n_test > 0].iloc[0].id
            sup_limit = results_df[results_df.n_test > 0].iloc[-1].id
            ax.set_xlim(inf_limit, sup_limit)
...
if __name__ == "__main__":
    ...
    pa = ProjectTestAnalysis(sys.argv[1], 2, root="/home/jovyan/work")
